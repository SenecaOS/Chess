...
public class Move {
	...
	public boolean isEnpassant() {
		if (game.getEnpassantSquareRow() == toRow && game.getEnpassantSquareCol() == toCol
				&& game.getDoublePushedPawn() != null) {
			return true;
		}
		return false;
	}
	...
	public void specialMove(Move move, Piece[][] board) {
		...
		if (piece instanceof Pawn) {
			if (move.isEnpassant()) {
				boardInstance.removeCapturedEnpassantPawn(game.getDoublePushedPawn().getRow(),
						game.getDoublePushedPawn().getCol());
			}

			if (isDoublePawnPush()) {
				game.setDoublePushedPawn((Pawn) piece);
			}
